...
with st.expander('Group By your columns'):
    ...
    with col3:
        operation=st.selectbox("Choose operation",options=['sum','max','min','mean','median','mode','count'])
    if(groupby_cols):
        ...
        graphs=st.selectbox('Choose your graphs',options=['line','bar','scattter','pie','sunburst'])
        if(graphs=='line'):
            ...
            st.plotly_chart(fig)
             ...
             y_axis=st.selectbox('Choose Y axis',options=list(result.columns))
             color=st.selectbox('Color Information',options=[None] +list(result.columns))
             facet_col=st.selectbox('Color Information',options=[None] +list(result.columns))
